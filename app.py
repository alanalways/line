...
# 如果想看更詳細的 DEBUG 訊息，可以改成 app.logger.setLevel(logging.DEBUG)
...
try:
    ...
    ai_client = OpenAI(api_key=grok_api_key, base_url=XAI_API_BASE_URL, timeout=httpx.Timeout(60.0, connect=10.0))
...
# (get_db_connection 和 init_db 函數保持不變，使用修正縮排後的版本)
...
# --- 背景處理函數 (加入圖片生成指令的詳細日誌) ---
def process_and_push(user_id, event):
    user_text_original = ""
    image_data_b64 = None
    is_image_gen_request = False # <--- 預設為 False
    image_gen_prompt = ""
    ...
    if isinstance(event.message, TextMessage):
        ...
        app.logger.info(f"收到文字訊息 (原始): {repr(user_text_original)}") # Log 原始 repr
        ...
        if starts_with_half:
            is_image_gen_request = True # <--- 設定為 True
            image_gen_prompt = cleaned_text[len(IMAGE_GEN_TRIGGER_HALF):].strip()
            app.logger.info(f"DEBUG: 偵測到圖片生成指令 (半形冒號): '{image_gen_prompt}'")

        # 檢查全形冒號觸發詞 (只有在半形不匹配時才檢查並設定)
        if not is_image_gen_request: # 避免重複設定
            ...
            if starts_with_full:
                ...
                app.logger.info(f"DEBUG: 偵測到圖片生成指令 (全形冒號): '{image_gen_prompt}'")

        # 最終確認 is_image_gen_request 的值
    ...
    # --- 處理圖片生成 ---
    if is_image_gen_request: # <--- 依賴這個 flag 是否被正確設為 True
        app.logger.info(f"判斷 is_image_gen_request 為 True，進入圖片生成流程...")
        if ai_client is None: final_response_message = TextSendMessage(text="[AI Client 初始化失敗]")
            ...
            try:
                ...
                # 【注意】以下呼叫仍基於假設，可能失敗
                response = ai_client.images.generate(model=IMAGE_GEN_MODEL, prompt=image_gen_prompt, n=1, size="1024x1024")
    ...
    # --- 處理文字或圖片描述 ---
    app.logger.info(f"判斷 is_image_gen_request 為 False，進入文字/圖片描述流程...")
    ...
    try:
        ...
        conn = get_db_connection()
        if conn:
             try: with conn.cursor() as cur: cur.execute(...); result = cur.fetchone(); ... # 同上版本
